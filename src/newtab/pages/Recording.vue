  ...
  <div class="p-5 w-full max-w-xl mx-auto">
    <div class="flex items-center">
      ...
      <div class="ml-4 flex-1 overflow-hidden">
        ...
        <p class="text-sm">Aufnahme</p>
        <p class="font-semibold text-xl leading-tight text-overflow">
          {{ state.name }}
        </p>
      </div>
    </div>
    <p class="mt-6 mb-2">Die Klickpfad Aufnahme hat begonnen. Wenn sie mit der Datenanfage fertig sind, können sie die
      Aufzeichnung mit dem roten 'Aufnahme' Button in dem Popup rechts oben auf der Seite beenden. </p>
    <!--
...
async function stopRecording() {
  ...
  try {
    ...
    postProcessRecordedWorkflow(state)
...
function postProcessRecordedWorkflow(state) {
  ...
  // Insert 'wait-for-input' signals after any password input blocks
  for (const flow of flows) {
    console.log('flow', flow)
    if (flow.description?.toLowerCase() === 'passwort' || flow.description?.toLowerCase() === 'password') {
      waiting = true;
      flows.splice(flows.indexOf(flow) + 1, 0, {
        id: 'javascript-code',
        description: "Send wait-for-input signal to backend",
        data: {
  ...
  if (!waiting) {
    flows.push({
      id: 'javascript-code',
      description: "Send completed signal to backend",
      data: {
