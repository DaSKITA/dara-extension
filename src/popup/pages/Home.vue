<template>
  <div :class="[!showTab ? 'h-48' : 'h-56']" class="bg-accent rounded-b-2xl absolute top-0 left-0 w-full"></div>
  <div :class="[!showTab ? 'mb-6' : 'mb-2']" class="dark placeholder-black relative z-10 text-white px-5 pt-8">
    <div class="flex items-center mb-4">
      <h1 class="text-xl font-semibold text-white">DARA (Automa)</h1>
      <div class="flex-grow"></div>
      <ui-button v-tooltip.group="
        t(`home.elementSelector.${state.haveAccess ? 'name' : 'noAccess'}`)
      " icon class="mr-2" @click="initElementSelector">
        <v-remixicon name="riFocus3Line" />
      </ui-button>
      <ui-button v-tooltip.group="t('common.dashboard')" icon :title="t('common.dashboard')" @click="openDashboard('')">
        <v-remixicon name="riHome5Line" />
      </ui-button>
    </div>
    <div class="flex">
      <ui-input v-model="state.query" :placeholder="`${t('common.search')}...`" autocomplete="off"
        prepend-icon="riSearch2Line" class="w-full search-input" />
    </div>
    <ui-tabs v-if="showTab" v-model="state.activeTab" fill class="mt-1" @change="onTabChange">
      <ui-tab value="local">
  ...
  <home-team-workflows v-if="state.retrieved" v-show="state.activeTab === 'team'" :search="state.query" />
  <div v-if="state.activeTab !== 'team'" class="px-5 z-20 relative pb-5 space-y-2">
    ...
    <div v-if="pinnedWorkflows.length > 0" class="mt-1 mb-4 border-b pb-4">
      ...
      <home-workflow-card v-for="workflow in pinnedWorkflows" :key="workflow.id" :workflow="workflow"
        :tab="state.activeTab" :pinned="true" class="mb-2" @details="openWorkflowPage"
        @update="updateWorkflow(workflow.id, $event)" @execute="executeWorkflow" @rename="renameWorkflow"
        @delete="deleteWorkflow" @toggle-pin="togglePinWorkflow(workflow)" />
    </div>
    <home-workflow-card v-for="workflow in workflows" :key="workflow.id" :workflow="workflow" :tab="state.activeTab"
      :pinned="state.pinnedWorkflows.includes(workflow.id)" @details="openWorkflowPage"
      @update="updateWorkflow(workflow.id, $event)" @execute="executeWorkflow" @rename="renameWorkflow"
      @delete="deleteWorkflow" @toggle-pin="togglePinWorkflow(workflow)" />
    <div v-if="state.showSettingsPopup"
      ...
      <p class="leading-tight text-sm">
        ...
        background in the
        <a href="https://docs.automa.site/workflow/settings.html#workflow-execution" class="font-semibold underline"
          target="_blank">
          workflow settings.
        </a>
      </p>
      <v-remixicon name="riCloseLine" class="absolute dark:text-gray-600 text-gray-300 top-2 right-2 cursor-pointer"
        size="20" @click="closeSettingsPopup" />
    </div>
  </div>
</template>
<script setup>
